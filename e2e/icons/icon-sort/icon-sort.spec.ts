...
test.describe('Icon-Sort Story', () => {
	const buttonText = 'Icon-Sort Default'
	const disableControlName = 'disabled'
	const loadingControlName = 'loading'
	const placeHolderControlName = 'Edit string...'
	const consoleActionMessage = 'Component: Icon-Sort - triggered action: Clicked'

	test.beforeEach(async ({ page }) => {
		await page.goto(`${STORYBOOK_URL}/${STORYBOOK_DOCS_PATH}/library-icons-icon-sort--docs`)
	})

	test('Should have name Icon-Sort', async ({ page }) => {
		await expect(sbLocatorGetHeading(page, 'Icon-Sort')).toHaveText('Icon-Sort')
	})
	...
	test('Should have updated label', async ({ page }) => {
		const buttonText = 'New Icon-Sort Default'
